...
class CarlaEnvironment():
    ...
    def __init__(self, client, world, town, checkpoint_frequency=100, continuous_action=True, max_timestep=20000) -> None:
    ...
    def reset(self):
        ...
        try:
            ...
            while(len(self.env_camera_obj.captured_images) == 0):
                time.sleep(0.0001)
            self.third_person_image_obs = self.env_camera_obj.captured_images.pop(-1)
            ...
            self.target_speed = 18 #km/h
            self.max_speed = 35.0
            self.min_speed = 15.0
            self.max_distance_from_center = 10
            ...
            return [self.image_obs, self.navigation_obs, self.third_person_image_obs]
...
# ----------------------------------------------------------------
    ...
    def step(self, action_idx):
        ...
        if self.timesteps >= self.max_timestep:
            done = True
        ...
        return [self.image_obs, self.navigation_obs, self.third_person_image_obs], reward, done, [self.distance_covered, self.center_lane_deviation]
